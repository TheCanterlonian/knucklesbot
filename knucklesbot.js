...
var helpText = [
  "Knucklesbot help (good luck, this bot is perpetually broken):",
  "	to prevent a user from using the bot:",
  "		`knuckles ignore <user>`",
  "	to remove knucklesbot's messages:",
  "		`knuckles remove <number>`",
  "	to allow a user to use knucklesbot again:",
  "		`knuckles allow <user>`",
  "	to get the list currently ignored users' ids:",
  "		`knuckles ignorelist`",
  "to invite the developer to your server (if the bot has invite perms)",
  "	`knuckles devhelp`",
  "to add/remove channel logging",
  ...
  "",
  "any message with `knuckles` in it will be detected, and will send a randomized i" +
      "mage if the user/channel is not banned from the bot",
  "",
  "PM <@123601647258697730> to submit a PNG image for knuckles bot or a link to a v" +
      "ideo about knuckles."
];
...
bot.on('ready', () => {
  ...
  bot
    .users
    .get("123601647258697730")
    .sendMessage(emoji.get(":white_check_mark:") + "Knucklesbot loaded.");
});
bot.on('message', msg => {
  ...
  if (msg.content.toUpperCase().includes("KNUCKLES") || msg.isMentioned(bot.user)) {
    ...
    if (params[0] === "help") {
      ...
      msg
        .author
        .send(helpTextCombined, {split: true});
      ...
      msg
        .channel
        .sendEmbed({title: "imgs", description: images, color: 0xff1111});
      ...
      var ment = msg
        ...
        .first();
      msg
        .reply(`you got lucky,  ${ment.username}`)
        .catch(errorHandler);
      ...
      var channel = params[1] !== undefined
        ? bot
          .channels
          .get(params[1])
        : msg.channel;
      ...
      logger("INFO", 'channelignore updated for ' + bot.guilds.get(msg.guild.id).name + ', [' + channelignore[msg.guild.id] + '] (updated by ' + (bot.guilds.get(msg.guild.id).members.get(msg.author.id).nickname !== (undefined || null)
        ? bot.guilds.get(msg.guild.id).members.get(msg.author.id).nickname
        : msg.author.username) + ')', msg.channel, msg);
      ...
      var channel = params[1] !== undefined
        ...
        : bot
          .channels
          .get(channelignore[msg.guild.id][0]);
      msg
        .reply(`removed ${channel.name} as an ignored channel`)
        .catch(errorHandler);
      var tempCh = channelignore[msg.guild.id];
      if (tempCh === undefined) {
        tempCh = [];
      ...
      logger("INFO", 'ignorelist updated for ' + bot.guilds.get(msg.guild.id).name + ', [' + serverignore[msg.guild.id] + '] (updated by ' + (bot.guilds.get(msg.guild.id).members.get(msg.author.id).nickname !== (undefined || null)
        ? bot.guilds.get(msg.guild.id).members.get(msg.author.id).nickname
        : msg.author.username) + ')', msg.channel, msg);
      ...
      if (wj) {
        ...
        msg
          .channel
          .send(emoji.get("regional_indicator_x"));
      } else {
        wj = true;
        msg
          .channel
          .send(emoji.get("white_check_mark"));
      }
      ...
      for (i = 0; i < serverignore[msg.guild.id].length; i++) {
        var e = msg
          ...
          .guild
          .members
          .get(serverignore[msg.guild.id][i].toString());
        if (e !== undefined) {
          list.push(e.nickname !== undefined
            ? e.nickname
            : e.user.username);
        }
      }
      ...
      logger("INFO", 'loglist updated for ' + bot.guilds.get(msg.guild.id).name + ', [' + bot.channels.get(loglist[msg.guild.id]).name + '] (updated by ' + (bot.guilds.get(msg.guild.id).members.get(msg.author.id).nickname !== (undefined || null)
        ? bot.guilds.get(msg.guild.id).members.get(msg.author.id).nickname
        : msg.author.username) + ')', msg.channel, msg);
      ...
      var channel = params[1] !== undefined
        ...
        : bot
          .channels
          .get(loglist[msg.guild.id][0]);
      msg
        .reply(`removed ${channel.name} from the logger`)
        .catch(errorHandler);
      var tempLog = loglist[msg.guild.id];
      if (tempLog === undefined) {
        tempLog = [];
...
function logger(eventType, message, channel, msg) {
  ...
  if (msg !== undefined) {
    console.log(eventType + " ::: " + hrs + ":" + mins + ":" + secs + " ::: " + message + " (" + (msg.author.username !== undefined
      ? msg.author.username
      : "undefined") + " @" + (msg.guild.name !== undefined
      ? msg.guild.name
      : "undefined") + ": #" + (channel.name !== undefined
      ? msg.channel.name
      : "undefined") + ")");
          ...
          if (msg.guild.channels.get(channel.id) !== undefined) {
            bot
              ...
              .sendEmbed({
                ...
                description: message,
                footer: {
                  text: (msg.guild.members.get(msg.author.id) !== undefined
                    ? (msg.guild.members.get(msg.author.id).nickname + " ")
                    : (msg.author !== undefined
                      ? (msg.author.username + " ")
                      : "")) + "at " + hrs + ":" + mins + ":" + secs + " in #" + channel.name,
                  icon_url: msg.author !== undefined
